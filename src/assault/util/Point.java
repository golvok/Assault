...
public class Point {
	...
	private static double magnitudeSquared(Point p) {
		return pow(p.getX(), 2) + pow(p.getY(), 2);
	}

	public static float magnitudeSquaredf(Point p) {
		return (float) magnitudeSquared(p);
	}
	...
	public static Point project(Point source, Point wall) {
		return multiply(wall, dotProduct(wall, source) / magnitudeSquaredf(wall));
	}
