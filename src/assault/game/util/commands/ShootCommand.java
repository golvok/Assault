 ...
 * @author 088241930
 */
public class ShootCommand extends ACommand implements MouseCommand, TargetCommand {
	...
	public ShootCommand(String name, ResourcePreloader rp, char hotKey, AWeapon aw) {
		this(name, rp, hotKey);
		weapon = aw;
	}
	...
	public void executeOn(AObject[] shooters, AObject target) {
		if (target != null) {
			for (int i = 0; i < shooters.length; i++) {
				if (shooters[i] instanceof AUnit) {
					if (weapon != null) {
						((AUnit) shooters[i]).shootBulletAt(target, weapon);
					} else if (mointPoint >= 0) {
						((AUnit) shooters[i]).shootBulletAt(target, mointPoint);
					}
				}
			}
		}
